...
if  (isset($_POST["submit"]) ){
    ...
    if(isset($tennv) && isset($ngaysinh) && isset($type)){
        ...
        $sql = "UPDATE `user` SET `tennv` = '$tennv', `ngaysinh` = '$ngaysinh', `type` = '$type'
        WHERE `user`.`manv`='$manv'";
...
<body>
    ...
    <div class="container">  
        <form action="" method="POST" enctype="multipart/form-data">
            <h1>Cập nhật User</h1><br>
            
            <input placeholder="Họ và Tên" type="text" id="ten" name="ten" tabindex="1" value="<?= $row["tennv"]?>">
            

            
            <input placeholder="Ngày sinh" type="date" id="date" name="date" tabindex="1" value="<?= $row["ngaysinh"]?>">
            

            
            <input placeholder="Chức vụ" type="text" id="type" name="type" tabindex="1" value="<?= $row["type"]?>"><br>
