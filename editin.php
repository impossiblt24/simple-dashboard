...
$makho = (int) $_GET['id'];
$sql = "SELECT * FROM `inventory` WHERE `makho` = '$makho'";
...
if  (isset($_POST["submit"]) ){
    ...
    if(isset($tensp) && isset($size) && isset($soluong)){
        $sql = "UPDATE `inventory` SET `tensp` = '$tensp', `size` = '$size', `soluong` = '$soluong'
        WHERE `inventory`.`makho` = '$makho'";
...
<body>
    ...
    <div class="container">  
        ...
        <button type="submit" name="submit">Sửa Hàng Tồn Kho</button>
            
        </form>
    </div>

    <button class="back" onclick= "window.location.href='inventory.php'">Back</button>
